        ...
        if (response.success) {
          if (response.errors && response.errors.length > 0) {
            ...
            if (response.importedCount > 0) {
              toast.success(
                `${response.importedCount} Candidate(s) Successfully Imported`
              );
            }
  ...
  const csvtemplate = async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.1.31:8000/api/candidate/download-csv-candidate",
        {
          method: "GET",
          headers: {
    ...
    <Layout>
      ...
      <section className="flex flex-col flex justify-center h-full gap-6 items-center">
        ...
        <form
          ...
          className="w-full max-w-sm bg-white rounded-lg p-6 mb-6"
        >
          <div className="mb-4">
            <label
              htmlFor="file"
              ...
              <thead>
                <tr>
                  {csvData[0].map((value, index) => (
                    <th
                      key={index}
                      className="border px-4 py-2 text-gray-600 font-semibold bg-gray-100"
                    >
                      {value}
                    </th>
                  ))}
